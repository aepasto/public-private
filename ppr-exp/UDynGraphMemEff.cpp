...
UDynGraphMemEff::UDynGraphMemEff() :
...
bool UDynGraphMemEff::add_edge(const int source, const int destination) {
...
bool UDynGraphMemEff::remove_edge(const int source, const int destination) {
...
int UDynGraphMemEff::num_edges() const {
    return num_edges_;
}

int UDynGraphMemEff::num_nodes() const {
    return num_nodes_;
}

void UDynGraphMemEff::neighbors(const int source, vector<int>* vec) const {
    vec->clear();
    if (node_map_.find(source) != node_map_.end()) {
...
void UDynGraphMemEff::nodes(vector<int>* vec) const {
    vec->clear();
    vec->assign(nodes_.begin(), nodes_.end());
}

int UDynGraphMemEff::degree(const int source) const {
    if (node_map_.find(source) == node_map_.end()) {
        return 0;
    }
...
void UDynGraphMemEff::edges(vector<pair<int, int> >* vec) const {
    vec->clear();
